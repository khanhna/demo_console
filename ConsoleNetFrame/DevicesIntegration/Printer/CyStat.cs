...
namespace ConsoleNetFrame.DevicesIntegration.Printer
{
    ...
    int dwCmdLen)
        {
            byte[] ByteArray = System.Text.Encoding.GetEncoding(65001).GetBytes(Cmd);
            GCHandle gchCmd = GCHandle.Alloc(ByteArray, GCHandleType.Pinned);
            long lpCmd = gchCmd.AddrOfPinnedObject().ToInt64();
            IntPtr myIntPtrValue = new IntPtr(lpCmd);
            int result = SetCommand_(lPortNum, myIntPtrValue, dwCmdLen);
            gchCmd.Free();
            return result;
        }

        public int GetCommand(int lPortNum, string Cmd, int dwCmdLen, ref StringBuilder rb, int dwRetBuffSize, ref int dwRetLen)
        {
            byte[] ByteArray;
            ByteArray = System.Text.Encoding.GetEncoding(65001).GetBytes(Cmd);
            GCHandle gchCmd = GCHandle.Alloc(ByteArray, GCHandleType.Pinned);
            GCHandle gchRetLen = GCHandle.Alloc(dwRetLen, GCHandleType.Pinned);
            IntPtr lpCmd = gchCmd.AddrOfPinnedObject();
            IntPtr lpdwRetLen = gchCmd.AddrOfPinnedObject();
            int result = GetCommand_(lPortNum, lpCmd, dwCmdLen, ref rb, dwRetBuffSize, out lpdwRetLen);
        ...
        public int GetCommandEX(int lPortNum, string Cmd, int dwCmdLen, ref StringBuilder rb, int dwRetBuffSize)
        {
            ...
            GCHandle gchCmd = GCHandle.Alloc(ByteArray, GCHandleType.Pinned);
            IntPtr lpCmd = gchCmd.AddrOfPinnedObject();
            int result = GetCommandEX_(lPortNum, lpCmd, dwCmdLen, rb, dwRetBuffSize);
            gchCmd.Free();
            return result;
        }
        ...
        public int SetFirmwDataWrite(int lPortNum, byte[] byteArray, int dwLen)
        {
            GCHandle gchCmd = GCHandle.Alloc(byteArray, GCHandleType.Pinned);
            IntPtr lpCmd = gchCmd.AddrOfPinnedObject();
            int result = SetFirmwDataWrite_(lPortNum, lpCmd, dwLen);
            gchCmd.Free();
            return result;
        }
        ...
        public static int SetColorDataVersion(int lPortNum, string Cmd, int dwCmdLen)
        {
            ...
            GCHandle gchCmd = GCHandle.Alloc(ByteArray, GCHandleType.Pinned);
            IntPtr lpCmd = gchCmd.AddrOfPinnedObject();
            int result = SetColorDataVersion_(lPortNum, lpCmd, dwCmdLen);
            gchCmd.Free();
            return result;
        }
        ...
        public static int SetColorDataWrite(int lPortNum, byte[] byteArray, int dwLen)
        {
            GCHandle gchCmd = GCHandle.Alloc(byteArray, GCHandleType.Pinned);
            IntPtr lpCmd = gchCmd.AddrOfPinnedObject();
            int result = SetColorDataWrite_(lPortNum, lpCmd, dwLen);
            gchCmd.Free();
            return result;
        }
