...
public class DevMode
{
    ...
    public static void PrintSettingRefactored(ref PRINT_SETTING prnSet, ref PrintDocument pd)
    {
        try
        {
            ...
            if (DocumentProperties(0, hPrinter, PrinterName, lpDevModeDmW, IntPtr.Zero, DM_OUT_BUFFER) < 0)
            {
                GlobalFree(lpDevModeDmW);
                PrinterModule.ClosePrinter(hPrinter);
                return;
            ...
            Dev2MemCopy(lpDevModeDmW, ref dmw, devModeSize);
            PrinterModule.ClosePrinter(hPrinter);
            pd.PrinterSettings.SetHdevmode(lpDevModeDmW);
            GlobalFree(lpDevModeDmW);
        }
        catch (Exception)
        {
